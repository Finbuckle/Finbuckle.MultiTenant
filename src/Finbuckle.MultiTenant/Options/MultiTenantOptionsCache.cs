...
/// <summary>
/// Adds, retrieves, and removes instances of TOptions for a given tenant.
/// </summary>
public class MultiTenantOptionsCache<TOptions>
    where TOptions : class
{
    ...
    /// <param name="tenantId">The ID of the tenant which will have its options retrieved or added.</param>
    ...
    public TOptions GetOrAdd(string? name, string? tenantId, Func<TOptions> createOptions)
    {
    ...
    /// <summary>
    /// Tries to add a new option to the cache for the current tenant.
    /// </summary>
    /// <param name="name">The options name.</param>
    /// <param name="tenantId">The ID of the tenant which will have its options added.</param>
    /// <param name="options">The options instance.</param>
    /// <returns>True if the options was added to the cache for the current tenant.</returns>
    public bool TryAdd(string? name, string? tenantId, TOptions options)
    {
    ...
    /// </summary>
    /// <param name="name">The options name.</param>
    /// <param name="tenantId">The ID of the tenant which will have its options removed.</param>
    /// <returns>True if the options was removed from the cache for the current tenant.</returns>
    public bool TryRemove(string? name, string? tenantId)
    {
        name = name ?? Microsoft.Extensions.Options.Options.DefaultName;
        var cache = map.GetOrAdd(tenantId ?? "", new OptionsCache<TOptions>());
