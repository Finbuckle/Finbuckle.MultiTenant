...
namespace Finbuckle.MultiTenant.Blazor
{
    ...
    /// </summary>
    internal class BlazorMultiTenantMiddleware
    {
        private readonly RequestDelegate next;

        public BlazorMultiTenantMiddleware(RequestDelegate next)
        {
            this.next = next;
        }

        public async Task Invoke(HttpContext context, TenantSession tenantSession)
        {
            // Set the initial multitenant context into the Items collections.
            await HandleContextAsync(context, tenantSession);
        ...
        private static async Task HandleContextAsync(HttpContext context, TenantSession tenantSession)
        {
            if (!context.Items.ContainsKey(Constants.HttpContextMultiTenantContext))
            {
                ...
                tenantSession.SetValue<IMultiTenantContext>(Constants.SessionStorageMultiTenantContext, multiTenantContext);
