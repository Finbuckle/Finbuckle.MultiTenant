...
public class EfCoreStoreImmutableTenantShould : ImmutableTenantStoreTestBase, IDisposable
{
    private sealed class ImmutableStoreDbContext(DbContextOptions options)
        : EFCoreStoreDbContext<ImmutableTenantInfo, string>(options)
    {
    ...
    protected override async Task<IMultiTenantStore<ImmutableTenantInfo, string>> CreateTestStore()
    {
        ...
        var store = new EFCoreStore<ImmutableStoreDbContext, ImmutableTenantInfo, string>(dbContext);
    ...
    public async Task PersistAdditionalConstructorBoundProperties()
    {
        ...
        Assert.True(await store.AddAsync(new ImmutableTenantInfo("named-id", "named", "Named Tenant")));

        var tenant = await store.GetByIdentifierAsync("named");

        Assert.NotNull(tenant);
        Assert.Equal("named-id", tenant.Id);
        Assert.Equal("Named Tenant", tenant.Name);
    }
